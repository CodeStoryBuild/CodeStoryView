...
export class CstManager {
     ...
     */
    public async get_exe(onStatus?: (status: 'downloading' | 'extracting' | 'ready' | 'error') => void): Promise<string | null> {
        ...
        try {
            ...
            vscode.window.showErrorMessage(`Failed to download Codestory CLI: ${error}`);
        }
    ...
    private async getLocalExePath(): Promise<string | null> {
        ...
        // The standalone zip/tar contains a 'cst' folder with the binary inside
        const exePath = path.join(this.storageUri.fsPath, 'cst', exeName);
    ...
    private async downloadCst(onStatus?: (status: 'downloading' | 'extracting' | 'ready' | 'error') => void): Promise<string | null> {
        ...
        const latestRelease = sortedReleases.find(r => semver.gte(r.tag_name.replace(/^v/, ''), MINIMUM_CST_VERSION));
        
        if (!latestRelease) {
            throw new Error(`No release found matching minimum version ${MINIMUM_CST_VERSION}`);
        }
        ...
        if (!asset) {
            throw new Error(`Asset ${assetName} not found in release ${latestRelease.tag_name}`);
        }
        ...
        await this.extract(downloadPath, this.storageUri.fsPath);
    ...
    private async fetchReleases(): Promise<Release[]> {
        const res = await fetch("https://api.github.com/repos/CodeStoryBuild/CodeStoryCli/releases");
        if (!res.ok) {
