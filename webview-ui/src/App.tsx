...
function App() {
  ...
  const [cstStatus, setCstStatus] = useState<'idle' | 'downloading' | 'extracting' | 'ready' | 'error'>('idle');
  ...
  useEffect(() => {
    const savedProvider = localStorage.getItem('vibe_selected_provider') || 'openai';
    const savedModel = localStorage.getItem('vibe_selected_model') || 'gpt-4o';
          ...
          <div className="flex flex-col items-center gap-4 p-6 bg-card border border-border rounded-xl shadow-2xl animate-in fade-in zoom-in duration-200">
            ...
            <div className="flex flex-col items-center gap-1">
              <h3 className="text-lg font-semibold">
                {cstStatus === 'downloading' ? 'Downloading Codestory CLI...' : 'Extracting CLI...'}
              </h3>
